...
1. SSN Input: The customer service agent enters the customer's personal identification number (SSN) in their system, or the customer is prompted to input their SSN via an automated PBX system.
2. Authentication request: The customer service agent (if applicable) indicates whether the call was initiated by the customer or the company they represent, then initiates authentication request. 
