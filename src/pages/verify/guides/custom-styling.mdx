...
This feature is available out of the box for [Criipto Verify](/verify) and [Criipto Signatures](/signatures) applications. 

To use custom styling with [Age Verification applications](/verify/guides/age-verification/), you need to configure your own MitID provider. 
...
and supports `da`, `en`, `sv`, `se`, `fi`, `nb` and a `default` parameter when no language is matched.
