...
| `prompt`        | (optional) Specifies whether the user will be forced to re-authenticate. Possible values are `none` (default) and `login`. More information is available [here](/verify/guides/sso/#per-request-options). |
