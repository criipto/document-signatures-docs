...
- **Signature order is created** when you create a signature order with `createSignatureOrder` you send a long document blobs with it,
...
This protects the documents from any outside access to personal data, in accordance with Screms-II.
...
The `cleanupSignatureOrder` can be called an indefinite amount of times after closing the order and is idempotent.
...
<GraphQLExample example={{query: cancelSignatureOrderExample.query, variables: (data) => ({...cancelSignatureOrderExample.variables(data), retainDocumentsForDays: 1})}} />